(function(){
  ...
  function autoMarketAmount(value,marketCode){
    const market=MARKETS[marketCode||state.market]||MARKETS.INTL;
    const raw=convertEGP(value,market.code);
    if(market.code==='EG')return Number(value)||0;
    return roundUpSmart(raw*AUTO_PRICE_BUFFER,market.currency);
  }

  function money(value){
    const market=currentMarket();
    const amount=market.code==='EG'?Number(value||0):autoMarketAmount(value,market.code);
    return formatCurrency(amount,market.currency);
  }

  // Manual country prices always win. Otherwise: live FX + 5% safety buffer + smart rounding.
